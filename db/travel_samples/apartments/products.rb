...
category_taxons = Spree::Taxon.where(:taxonomy_id => category_taxonomy.id).where(:name => 'Apartment').first.self_and_descendants.to_a
shipping_category = Spree::ShippingCategory.first
properties = Spree::Property.all.to_a
amount_of_fake_products = 5 # HERE
available_on = Time.now - 1.day
product_type = Spree::ProductType.where(:name => 'apartment').first
apartment_calculator = Spree::TravelCalculator.find_by_name('Spree::CalculatorApartment')

### Destroying Products
apartments = Spree::Product.where(:product_type => Spree::ProductType.find_by_name('apartment')).destroy_all
...
amount_of_fake_products.times do
  ...
  the_name = "Apartment: " + Faker::Company.name
  ...
  product_attrs = {
    ...
    :calculator_id => apartment_calculator.id,
  }
